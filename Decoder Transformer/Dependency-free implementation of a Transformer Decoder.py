...
(other * self.data**(other-1),)) 
    def log(self): return Value(math.log(self.data), (self,), 
(1/self.data,)) 
...
matrix = lambda nout, nin, std=0.08: [[Value(random.gauss(0, std)) for _ in 
...
/ head_dim**0.5 for t in range(len(k_h))] 
            ...
            head_out = [sum(attn_weights[t] * v_h[t][j] for t in 
range(len(v_h))) for j in range(head_dim)] 
...
for step in range(num_steps): 
    ...
    keys, values = [[] for _ in range(n_layer)], [[] for _ in 
range(n_layer)] 
    losses = [] 
...
range(n_layer)] 
    ...
    for pos_id in range(block_size): 
        ...
        token_id = random.choices(range(vocab_size), weights=[p.data for p 
in probs])[0] 
